...
export default function Disputes() {
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      case 'REJECTED': return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-surface-muted text-ink">REJECTED</span>;
      default: return <span className="px-2 py-1 text-xs font-semibold rounded-full bg-surface-muted text-ink">{status}</span>;
    }
  };
  ...
  return (
    <div className="px-8 py-8 animate-fade-in space-y-6 relative">
      {toastMessage && (
        <div className="fixed top-4 right-4 z-[80] bg-ink text-white px-4 py-3 rounded shadow-lg animate-fade-in">
          {toastMessage}
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight text-ink">Payslip Disputes</h1>
          <p className="mt-1 text-sm text-ink-muted">Manage and resolve employee payslip issues</p>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 text-left">
              <thead className="bg-surface-muted">
                <tr>
                  ...
                  <th className="px-6 py-3 text-xs font-medium text-ink-muted uppercase">Reason</th>
                  <th className="px-6 py-3 text-xs font-medium text-ink-muted uppercase">Raised On</th>
                  <th className="px-6 py-3 text-xs font-medium text-ink-muted uppercase">Status</th>
                  {isOfficerOrAdmin && <th className="px-6 py-3 text-xs font-medium text-ink-muted uppercase text-right">Actions</th>}
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredDisputes.map(d => {
                  ...
                  return (
                    <tr key={d.id} className="hover:bg-surface-muted transition">
                      <td className="px-6 py-4 text-sm font-medium text-ink">{empName}</td>
                      <td className="px-6 py-4 text-sm text-ink-muted">{period}</td>
                      <td className="px-6 py-4 text-sm text-ink-muted max-w-xs" title={d.reason}>{truncatedReason}</td>
                      <td className="px-6 py-4 text-sm text-ink-muted">{new Date(d.createdAt).toLocaleDateString()}</td>
                ...
                {filteredDisputes.length === 0 && (
                  <tr>
                    <td colSpan={isOfficerOrAdmin ? 6 : 5} className="px-6 py-8 text-center text-ink-muted">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 animate-fade-in p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="p-6 border-b border-border">
              <h2 className="text-xl font-bold text-ink capitalize">{activeModal} Dispute</h2>
            </div>
            ...
            <div className="p-6">
              ...
              <textarea
                ...
                className="w-full border border-border-strong rounded-lg p-3 text-sm focus:ring-brand-500 focus:border-brand-500 outline-none"
                rows="4"
                placeholder="Explain the decision..."
              ></textarea>
            </div>

            <div className="p-4 bg-surface-muted border-t border-border flex justify-end gap-3">
              <button 
                onClick={closeModal} 
                disabled={submitting}
                className="px-4 py-2 text-ink-muted hover:bg-border rounded-md transition font-medium"
              >
                Cancel
              </button>
              <button 
