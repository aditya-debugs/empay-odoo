      ...
      <Card className="p-6">
        <div className="flex flex-col sm:flex-row gap-4 items-center">
          <input 
            ...
            className="border border-border-strong rounded-md px-4 py-2 focus:ring-brand-500 focus:border-brand-500 outline-none"
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 animate-fade-in">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full shadow-xl">
            <h3 className="text-lg font-bold text-ink mb-2">Confirm Process</h3>
            <p className="text-ink-muted mb-6">Process payroll for {month}? This cannot be undone.</p>
              ...
              return (
                <div key={idx} className="flex flex-col items-center gap-2 bg-white px-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-colors ${
                    active ? 'bg-brand-600 border-brand-600 text-white' : 'bg-white border-border-strong text-ink-soft'
            ...
            <table className="min-w-full divide-y divide-gray-200 text-left">
              <thead className="bg-surface-muted">
                <tr>
                  ...
                  <th className="px-6 py-3 text-xs font-medium text-ink-muted uppercase">Working Days</th>
                  <th className="px-6 py-3 text-xs font-medium text-ink-muted uppercase">Present</th>
                  <th className="px-6 py-3 text-xs font-medium text-ink-muted uppercase">LOP Days</th>
                  <th className="px-6 py-3 text-xs font-medium text-ink-muted uppercase">Gross</th>
                  <th className="px-6 py-3 text-xs font-medium text-ink-muted uppercase">Deductions</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {previewData.map((row, i) => {
                  ...
                  return (
                    <tr 
                      ...
                      className="hover:bg-surface-muted cursor-pointer transition"
                    >
                      <td className="px-6 py-4 text-sm font-medium text-ink">{row.employeeName}</td>
                ...
                {previewData.length === 0 && (
                  <tr>
                    <td colSpan="9" className="px-6 py-8 text-center text-ink-muted">No active employees found for preview.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 animate-fade-in p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-border flex justify-between items-center bg-surface-muted">
              <div>
                <h2 className="text-xl font-bold text-ink">Salary Breakdown</h2>
                <p className="text-sm text-ink-muted mt-1">{selectedEmployee.employeeName} — {month}</p>
              </div>
              <button 
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* Earnings */}
                <div>
                  <h3 className="text-lg font-semibold text-ink mb-4 border-b pb-2">Earnings</h3>
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between text-sm">
                      <span className="text-ink-muted">Overtime Bonus</span>
                    ...
                    <div className="pt-3 border-t mt-3 flex justify-between font-bold text-ink">
                ...
                {/* Deductions */}
                <div>
                  <h3 className="text-lg font-semibold text-ink mb-4 border-b pb-2">Deductions</h3>
                  <div className="space-y-3">
                    <div className="flex justify-between text-sm">
                      <span className="text-ink-muted">LOP Deduction</span>
                      <span className="font-medium text-red-600">{formatINR(selectedEmployee.lopDeduction)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-ink-muted">PF</span>
                      <span className="font-medium text-red-600">{formatINR(selectedEmployee.pfDeduction)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-ink-muted">ESIC</span>
                    ...
                    <div className="flex justify-between text-sm">
                      <span className="text-ink-muted">Professional Tax</span>
            ...
            <div className="p-6 bg-surface-muted border-t border-border flex justify-between items-center">
              <span className="text-ink-muted font-medium">Net Payable Salary</span>
