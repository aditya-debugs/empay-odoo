...
export default function CreateEmployeePage() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      setError(detail || err.message || 'Failed to create employee');
    } finally {
      setSubmitting(false);
    }
  }
      ...
      <div className="px-8 py-8">
        <Card className="mx-auto max-w-2xl p-8 text-center">
          ...
          <p className="mt-1 text-sm text-ink-muted">
            ...
            <span className="font-medium text-ink">{submitted.user.email}</span>. They will be
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs text-ink-muted">
                <span className="font-mono">[Company][First-2][Last-2][Year][Serial]</span> — final
                serial assigned on save.
              </div>
            </div>
        ...
        <Section title="Personal Information" icon={UserIcon}>
          <div className="md:col-span-2 flex flex-col gap-1.5 border-b border-ink-100 pb-4 mb-2">
            ...
            <div className="flex items-center gap-4">
              {form.avatarUrl ? (
                <img
                  src={import.meta.env.VITE_API_URL.replace('/api/v1', '') + form.avatarUrl}
                  alt="Avatar"
        ...
        <Section title="Employment Information" icon={Briefcase}>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex h-11 items-center gap-2 rounded-xl border border-border bg-surface-muted px-3.5 text-sm text-ink">
              ...
              <span>Employee</span>
              <span className="ml-auto text-xs text-ink-muted">Fixed role</span>
            </div>
          </div>
        </Section>
        ...
        <Section title="Salary Information" icon={IndianRupee}>
          <Input
            ...
            onChange={setEv('basicSalary')}
            required
          />
          <Input
            label="HRA"
            type="number"
            hint="Standard: 40% non-metro, 50% metro"
            value={form.hra}
            onChange={setEv('hra')}
          />
          <Input
            ...
            onChange={setEv('conveyance')}
          />
          <Input
            label="Special Allowance"
            type="number"
            value={form.specialAllowance}
            onChange={setEv('specialAllowance')}
          />
          <Input
            label="Other Allowances"
            type="number"
            value={form.otherAllowance}
            onChange={setEv('otherAllowance')}
          />
          <div className="flex flex-col gap-1.5">
          ...
          <Input
            ...
            onChange={setEv('professionalTax')}
            hint="Auto-populates by state once Settings are configured"
          />
        </Section>
