...
import { AlertCircle, CheckCircle, XCircle, ArrowLeft, MessageSquare } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col gap-6 px-8 py-8">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-ink-900">Payslip Disputes Queue</h1>
          <p className="text-ink-muted">Review and resolve issues raised by employees regarding their payslips.</p>
        </div>
      </div>
              ...
              disputes.map((dispute) => (
                <tr key={dispute.id} className="hover:bg-surface-muted/40 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    {dispute.payslip ? `${new Date(dispute.payslip.year, dispute.payslip.month - 1).toLocaleString('en', { month: 'short' })} ${dispute.payslip.year} (v${dispute.payslip.version})` : 'Unknown'}
                  </td>
                  ...
                  <td className="px-5 py-4 text-right">
                    {dispute.status === 'OPEN' ? (
                      ...
                      <Button variant="outline" size="sm" onClick={() => navigate(`/admin/payroll/payslip/${dispute.payslipId}`)}>
                        View Payslip
                      </Button>
                    )}
                  </td>
      ...
      {selectedDispute && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <Card className="w-full max-w-lg p-6 shadow-xl">
            <h2 className="text-lg font-bold text-ink-900 mb-2">Resolve Dispute</h2>
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <Button variant="danger" loading={resolving} onClick={() => handleResolve('REJECTED')}>
                Reject
              </Button>
              <Button variant="primary" loading={resolving} onClick={() => handleResolve('RESOLVED')}>
                Resolve & Close
              </Button>
            </div>
            <p className="mt-4 text-xs text-ink-muted">
              Note: If changes are needed, close this dispute and generate a new payrun version for this employee.
            </p>
          </Card>
        </div>
      )}
